...
console.log(supabaseUrl, supabasePublishableKey, hasValidCredentials);
...
type SubscriptionCallback = (event: AuthChangeEvent, session: AuthSession | null) => void;
...
class MockAuthService {
  ...
  constructor() {
    ...
    const stored = localStorage.getItem('cullr_mock_session');
    if (stored) {
      try {
        this.currentSession = JSON.parse(stored);
      } catch (e) {
        localStorage.removeItem('cullr_mock_session');
      }
    }
  }
  ...
  async signUp({ email, password }: { email: string; password?: string }) {
    ...
    // Simple verification
    const users = JSON.parse(localStorage.getItem('cullr_mock_users') || '[]');
    if (users.some((u: any) => u.email === email)) {
      return { data: { user: null, session: null }, error: new Error('User already exists in mock database') };
    }
    ...
    users.push(newUser);
    localStorage.setItem('cullr_mock_users', JSON.stringify(users));
    ...
    localStorage.setItem('cullr_mock_session', JSON.stringify(session));
  ...
  async signInWithPassword({ email, password }: { email: string; password?: string }) {
    ...
    if (!user) {
      return { data: { user: null, session: null }, error: new Error('Invalid email or password in mock database') };
    }
